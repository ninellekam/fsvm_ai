    ...
    <div ref={ref} className="fixed inset-0 -z-10 pointer-events-none" aria-hidden>
      ...
      <style>{`
        ...
        .fx3 {
          ...
          left: 50%; bottom: -12vmin;
          transform: translateX(-50%) translate3d(0, var(--p3, 0), 0);
          background: radial-gradient(closest-side, var(--c3), transparent 78%);
          opacity: .18;
        }
        @media (max-width: 768px) {
          .fx { filter: blur(70px); }
          .fx1 { width: 70vmin; height: 70vmin; left: -20vmin; top: 18vmin; }
          .fx2 { width: 70vmin; height: 70vmin; right: -24vmin; top: 8vmin; }
...
/* ===================== useAccentTheme (смена палитры по секции) ===================== */
function useAccentTheme() {
  useEffect(() => {
    const root = document.documentElement;
    const sections = Array.from(document.querySelectorAll<HTMLElement>("[data-c1]"));
    if (!sections.length) return;

    const setColorsFrom = (el: HTMLElement) => {
      const c1 = el.dataset.c1;
      const c2 = el.dataset.c2 || c1;
      const c3 = el.dataset.c3 || c1;
      if (c1) root.style.setProperty("--c1", c1);
      if (c2) root.style.setProperty("--c2", c2);
      if (c3) root.style.setProperty("--c3", c3);
    };

    // Инициализация — ближайшая видимая
    const initial = sections.find((s) => {
      const r = s.getBoundingClientRect();
      return r.top < window.innerHeight * 0.6 && r.bottom > window.innerHeight * 0.4;
    });
    if (initial) setColorsFrom(initial);
    ...
    const io = new IntersectionObserver(
      (entries) => {
        ...
        if (best?.target) setColorsFrom(best.target as HTMLElement);
      },
    ...
    <Card className="border-white/10 bg-black/40 backdrop-blur-xl overflow-hidden">
      <div className="relative aspect-[4/5] w-full">
        ...
        <div className="absolute inset-y-0 left-0 overflow-hidden" style={{ width: `${value}%` }}>
          <img src={after} alt={altAfter} className="h-full w-full object-cover" />
        </div>

        {/* разделитель/ползунок */}
        <div className="pointer-events-none absolute inset-y-0" style={{ left: `${value}%`, transform: "translateX(-50%)" }}>
          <div className="h-full w-px bg-white/70" />
          <div className="absolute top-1/2 -translate-y-1/2 -left-3">
            <div className="h-6 w-6 rounded-full bg-white text-black text-xs font-bold grid place-items-center shadow">
              ⇆
      ...
      {caption && <div className="px-4 pb-4 pt-3 text-sm text-white/80">{caption}</div>}
    </Card>
  );
}
...
const examples = [
  { before: "/images/3.jpg", after: "/images/skirt.jpg", title: "Юбка плетённая — стиль «Дюна»" },
  { before: "/images/2.jpg", after: "/images/5.png", title: "Мини с ремнём — горный сет" },
  { before: "/images/7.jpg", after: "/images/8.jpg", title: "Юбка с разрезом — Марокко" },
];
...
export default function Index() {
  ...
  return (
    <div className="relative min-h-screen bg-black text-white">
      ...
      <nav className="sticky top-0 z-40">
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          <div className="mt-3 mb-4 flex items-center justify-between rounded-2xl border border-white/10 bg-black/40 px-4 py-3 backdrop-blur-xl">
            <div className="flex items-center gap-3">
              <img src="/logo.svg" alt="Logo" className="h-8 w-8" />
              <span className="text-xl md:text-2xl font-extrabold tracking-tight">AI Fashion</span>
            </div>
      ...
      {/* HERO — используйте ваш фон /images/hero.svg */}
      <section
        data-c1="#7b5cff"
        data-c2="#ff6f6f"
        data-c3="#ffe259"
      ...
      >
        <div
          ...
          style={{
            backgroundImage:
              "linear-gradient(to top, rgba(0,0,0,.72), rgba(0,0,0,.35), transparent), url('/images/hero.svg')",
            backgroundSize: "cover",
            backgroundPosition: "center",
          ...
          <div className="relative z-10 mx-auto max-w-7xl h-full px-4 md:px-8">
            <div className="flex h-full items-end md:items-center">
              <div className="pb-10 md:pb-0 max-w-3xl">
                ...
                <p className="mt-4 text-white/80 text-base md:text-lg max-w-2xl">
                  Генерируйте фотосессии товаров в нужных стилях и локациях за минуты. Без логистики, модели и аренды — просто загрузите продукт.
                </p>
                <div className="mt-6 flex flex-col sm:flex-row gap-3">
                  <a
                    href="https://t.me/fsvm_aifashion_bot"
      ...
      {/* ДЛЯ БРЕНДОВ — две BeforeAfter карточки справа */}
      <section
        id="for-brands"
        data-c1="#6b7bff"
        data-c2="#ffa26f"
      ...
      >
        <div className="mx-auto max-w-7xl px-4 md:px-8 grid md:grid-cols-2 gap-10 items-start">
          <div>
            ...
            <p className="mt-4 text-white/80">
              Быстро проверяйте гипотезы и собирайте каталоги без студийной рутины. Единый стиль, повторяемые ракурсы, масштабируемая выдача.
            </p>
            <ul className="mt-6 space-y-3">
          ...
          {/* Две карточки Before/After, как в “Примерах” */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <BeforeAfter
              before="/images/3.jpg"
              after="/images/skirt.jpg"
              caption="Юбка плетённая — стиль «Дюна»"
            />
            <BeforeAfter
              before="/images/2.jpg"
              after="/images/5.png"
              caption="Мини с ремнём — горный сет"
              initial={50}
            />
          </div>
        </div>
      ...
      >
        <div className="mx-auto max-w-7xl px-4 md:px-8 grid md:grid-cols-2 gap-10 items-center">
          <div className="rounded-2xl overflow-hidden border border-white/10 bg-white/5">
            <img src="/images/studio-showcase.jpg" alt="" className="w-full h-full object-cover" />
          </div>
          <div>
            <h2 className="text-2xl md:text-3xl font-bold tracking-tight">
            ...
            <p className="mt-4 text-white/80">
              Сократите стоимость и сроки, сохранив контроль качества — шаблоны сетапов, пакетная обработка, готовые пресеты под площадки.
            </p>
            <ul className="mt-6 space-y-3">
              <li className="flex gap-3">
                <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-emerald-400/20 text-emerald-300 text-xs">✓</span>
                <span className="text-white/85">Повторяемые ракурсы и сетапы</span>
              </li>
              <li className="flex gap-3">
                <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-emerald-400/20 text-emerald-300 text-xs">✓</span>
                <span className="text-white/85">Пакетная обработка коллекций</span>
              </li>
              <li className="flex gap-3">
                <span className="mt-0.5 inline-flex h-5 w-5 items-center justify-center rounded-full bg-emerald-400/20 text-emerald-300 text-xs">✓</span>
                <span className="text-white/85">Экспорт под маркетплейсы и соцсети</span>
              </li>
            </ul>
          </div>
        </div>
      ...
      >
        <div className="mx-auto max-w-7xl px-4 md:px-8">
          <h2 className="text-2xl md:text-3xl font-bold text-center tracking-tight">Примеры съёмок</h2>
          <div className="mt-10 grid md:grid-cols-3 gap-6 md:gap-8">
            {examples.map((ex, idx) => (
              <BeforeAfter key={idx} before={ex.before} after={ex.after} caption={ex.title} />
            ))}
          </div>
        </div>
      </section>
      ...
      <section
        ...
        data-c2="#ff7f7f"
        data-c3="#ffd86b"
        className="py-16 md:py-20 bg-white/[0.03]"
      >
        <div className="mx-auto max-w-5xl px-4 md:px-8">
          <h2 className="text-2xl md:text-3xl font-bold text-center tracking-tight">FAQ</h2>
          <div className="mt-8 space-y-3">
            {[
              { q: "Как начать?", a: "Перейдите в Telegram‑бот, загрузите фото товара на нейтральном фоне, выберите стиль — и получите готовые кадры." },
              { q: "Сколько стоит?", a: "На старте — бесплатно с лимитами. Далее — гибкие тарифы, в среднем до 10× дешевле классической студии." },
              { q: "Где обрабатываются данные?", a: "Генерация выполняется на наших серверах с GPU. Доступны политики хранения и удаления исходников." },
              { q: "Можно ли кастомизировать стиль?", a: "Да. Поддерживаем собственные пресеты, фирменные сетапы и сцены под бренд‑гайд." },
            ].map((item, i) => (
              <details key={i} className="group rounded-xl border border-white/10 bg-black/40 backdrop-blur-xl p-5">
                <summary className="cursor-pointer list-none font-semibold flex items-center justify-between">
                  <span>{item.q}</span>
                  <span className="ml-4 text-white/60 group-open:rotate-180 transition">⌄</span>
                </summary>
      ...
      <footer className="py-10">
        <div className="mx-auto max-w-7xl px-4 md:px-8 flex flex-col md:flex-row items-center justify-between gap-4 text-sm text-white/70">
          ...
          <div className="flex items-center gap-4">
            <a href="https://t.me/fsvm_aifashion_bot" target="_blank" rel="noopener noreferrer" className="hover:text-white">
              Telegram‑бот
            </a>
            <a href="#faq" className="hover:text-white">FAQ</a>
            <a href="#examples" className="hover:text-white">Примеры</a>
          </div>
        </div>
        <MadeWithDyad />
      </footer>
